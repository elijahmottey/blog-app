...
import { Edit, Trash2, Eye, Plus, Search, RefreshCw, MessageSquare, Heart, Bookmark } from 'lucide-react';
import {
  Button,
  TextField,
  InputAdornment,
...
export const PostsManagement: React.FC = () => {
  ...
  const filteredPosts = posts.filter(post => {
    ...
    if (selectedTimeRange !== 'all' && post.createdAt) {
      const postDate = new Date(post.createdAt).getTime();
      const now = new Date().getTime();
      const diffHours = (now - postDate) / (1000 * 60 * 60);

      if (selectedTimeRange === 'new') {
    ...
    <div className="aws-spacing-y-sm   ">
      ...
      >
        ...
        >
          ...
          <option value="new">Past 24 Hours</option>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-1">
                            <Bookmark size={14} style={{ color: theme.palette.primary.main }} />
                            <span className="aws-text-body" style={{ color: theme.palette.text.secondary, fontSize: '0.75rem', fontWeight: 600 }}>
                              {/* @ts-ignore */}
                              {post.saves ?? post.bookmarks ?? 0}
                            </span>
                          </div>
                          <div className="flex items-center gap-1">
